/******************************************************
 * Analise de jogos Lotofácil na planilha
 *
 ...
 *  - Colunas J1..J{n} na aba "Resultados" (acertos por jogo)
 *  - Aba "Resumo_Jogos"
 *  - Aba "Concursos_14plus"
 *  - Aba "SUGESTOES_DIA" (para o Make/IA ler e enviar por e-mail)
 ******************************************************/

function atualizarAnaliseLoto() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const dbSheetName = 'Resultados';      // aba de resultados históricos
  const jogosSheetName = 'JOGOS';        // aba com os jogos
  const resumoSheetName = 'Resumo_Jogos';
  const conc14SheetName = 'Concursos_14plus';
  const sugestoesSheetName = 'SUGESTOES_DIA';

  /*******************
   * 1) Ler aba de resultados ("Resultados")
   *******************/
  const dbSheet = ss.getSheetByName(dbSheetName);
  if (!dbSheet) {
    throw new Error('Aba "' + dbSheetName + '" não encontrada. Verifique o nome exato da aba de resultados.');
  }
  ...
  if (lastRow < 2) {
    throw new Error('Aba "' + dbSheetName + '" tem cabeçalho, mas nenhuma linha de concurso (linha 2 em diante está vazia).');
  }
  if (lastCol < 17) {
    throw new Error('Aba "' + dbSheetName + '" precisa ter pelo menos até a coluna Q (d15).');
  }
  ...
  // Opcional: validar cabeçalho mínimo
  const a1 = String(header[0] || '').toLowerCase().trim();
  const b1 = String(header[1] || '').toLowerCase().trim();
  if (a1 !== 'concurso' || b1 !== 'data') {
    throw new Error('Linha 1 da aba "' + dbSheetName + '" deve ter "concurso" na coluna A e "data" na coluna B.');
  }

  /*******************
   * 2) Ler / criar aba JOGOS
   *******************/
  const defaultJogos = [
  ...
  /*******************
   * 3) Calcular acertos (concurso x jogo)
   *******************/
  ...
  /*******************
   * 4) Escrever acertos em "Resultados" (J1..J{n})
   *******************/
  ...
  /*******************
   * 7) Gerar aba SUGESTOES_DIA (TOP por histórico de 14+)
   *******************/
  let sugestoesSheet = ss.getSheetByName(sugestoesSheetName);
  if (!sugestoesSheet) {
    sugestoesSheet = ss.insertSheet(sugestoesSheetName);
  } else {
    sugestoesSheet.clear();
  }

  const sugestoesHeader = [
    'DataExecucao',
    'JogoID',
    'Origem',
    'n1','n2','n3','n4','n5','n6','n7','n8','n9','n10','n11','n12','n13','n14','n15'
  ];
  sugestoesSheet.getRange(1, 1, 1, sugestoesHeader.length).setValues([sugestoesHeader]);

  // Score = Q_>=14 (cont14 + cont15)
  const jogosScore = [];
  for (let j = 0; j < numJogos; j++) {
    const jogoID = jogosDefinidos[j][0];
    const nums = jogosDefinidos[j][1];
    const score14plus = cont14[j] + cont15[j];
    jogosScore.push({
      id: jogoID,
      nums: nums,
      score: score14plus
  ...
  // Ordenar descrescente por score
  jogosScore.sort(function(a, b) {
    return b.score - a.score;
  });

  // Quantos jogos sugerir por dia
  const NUM_SUGESTOES = 5; // ajuste aqui se quiser mais/menos sugestões

  const hoje = new Date();
  const linhasSugestoes = [];

  for (let i = 0; i < Math.min(NUM_SUGESTOES, jogosScore.length); i++) {
    const jInfo = jogosScore[i];
    const row = [
      hoje,
      jInfo.id,
      'TOP_HISTORICO_14+'
    ].concat(jInfo.nums);
    linhasSugestoes.push(row);
  }

  if (linhasSugestoes.length > 0) {
    sugestoesSheet.getRange(2, 1, linhasSugestoes.length, sugestoesHeader.length)
                  .setValues(linhasSugestoes);
  }
}
